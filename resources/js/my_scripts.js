...
function viewStudentStats(id, toggle) {
	...
	if (toggle == 0) {
		idcode.style.height = "0px";
		idcode.style.marginBottom = "0px";
		idcode.style.visibility = "hidden";

	} else {
		idcode.style.height = "50px";
	...
	if (id == "student_status" && toggle == 1) {
		...
		idcode.style.marginBottom = "80px";
	}
}
...
function loadStatsPage() {
	...
	for (let row of mytable.rows) {
		...
		if (line > 1) {
			...
			tempOwn = parseFloat(row.cells[2].innerText);
			tempOpponent = parseFloat(row.cells[3].innerText);
			console.log(tempOpponent);
			if (tempOwn > tempOpponent) {
				row.cells[4].innerText = "CUBuffs";
				wins += 1;
			} else {
				row.cells[4].innerText = opponent;
			}
		}
		line += 1;
	}
